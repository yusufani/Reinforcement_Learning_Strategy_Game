...
class Env:
    ...
    SIZE = 10
    OBSERVATION_SPACE_VALUES = SIZE * SIZE + (NUMBER_OF_AGENT_PLAYER + 1) * (
            NUMBER_OF_ENEMY_PLAYER + 1) + NUMBER_OF_AGENT_PLAYER + NUMBER_OF_ENEMY_PLAYER + 2  # +2 for base health
    ACTION_SPACE_SIZE = 9
    ...
    def step(self, actions, step, episode):
        ...
        for idx, val in enumerate(move_agent_players):
            if val:
                self.agent_players[idx].move(agent_directions[idx][0], agent_directions[idx][1])
    ...
    def get_observations(self, get_only_map=False):
        map = np.zeros((self.SIZE, self.SIZE), dtype=np.uint8)
        ...
        if get_only_map:
            return map
        distances = np.zeros((len(self.agent_players) + 1, len(self.enemy_players) + 1))
        for i, agent in enumerate(self.agent_players):
            for j, enemy in enumerate(self.enemy_players):
                distances[i][j] = self.get_distance(agent, enemy)
            distances[i][-1] = self.get_distance(agent, self.enemy_base)
        for idx, val in enumerate(self.enemy_players):
            distances[-1][idx] = self.get_distance(val, self.agent_base)
        agent_healths = np.zeros(self.NUMBER_OF_AGENT_PLAYER)
        for idx, val in enumerate(self.agent_players):
            agent_healths[idx] = val.health
        enemy_healths = np.zeros(self.NUMBER_OF_ENEMY_PLAYER)
        for idx, val in enumerate(self.enemy_players):
            enemy_healths[idx] = val.health
        base_healts = np.array([self.agent_base.health, self.enemy_base.health])
        obs = np.concatenate([map.flatten(), distances.flatten(), agent_healths, enemy_healths, base_healts])
        # print(obs.shape)
        return obs
